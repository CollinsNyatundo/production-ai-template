...
class ContextManager:
    ...
    async def pack_context(self, documents: List[SearchDocument], token_budget: int = None, model: str = None) -> List[SearchDocument]:
        ...
        for doc in sorted_docs:
            ...
            if current_tokens + doc_tokens <= token_budget:
                ...
                logger.info(f"Document {doc.metadata.get('source')} added. Tokens: {doc_tokens}. Total: {current_tokens}")
            else:
                ...
                if remaining_tokens > 20: # Only truncate if it is worth adding a snippet
                    ...
                    logger.info(f"Document {doc.metadata.get('source')} truncated to fit remaining {remaining_tokens} tokens.")
