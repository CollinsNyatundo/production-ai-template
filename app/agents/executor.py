...
class AgentExecutor:
    ...
    async def execute_trajectory(self, session_id: str, query: str, actor_permission: str = "low") -> Dict[str, Any]:
        ...
        while step_count < self.max_turns and not agent_state.get("completed"):
            ...
            observation = ""
            if action_tool:
                await lifecycle_hooks.emit("on_tool_execute", session_id=session_id, tool_name=action_tool, args=action_args)
                try:
                    observation_raw = await tool_registry.execute_tool(action_tool, action_args, actor_permission)
        ...
        if agent_state.get("completed"):
            # Clear snapshot from SQLite database
            pass
