...
class Settings(BaseSettings):
    ...
    anthropic_api_key: str = Field(
        default="mock-key", validation_alias="ANTHROPIC_API_KEY"
    )
    ...
    jwt_secret: str = Field(
        default="production-super-secret-key-change-me", validation_alias="JWT_SECRET"
    )
    ...
    circuit_breaker_failure_threshold: int = Field(
        default=5, validation_alias="CB_FAILURE_THRESHOLD"
    )
    circuit_breaker_recovery_timeout: int = Field(
        default=30, validation_alias="CB_RECOVERY_TIMEOUT"
    )
